...
void Core::GameManager::AddGameObjectToScene(const std::string& p_name, std::shared_ptr<GameObject> p_gameObject, const unsigned int p_scene)
{
...
unsigned int Core::GameManager::GetActiveSceneNumber() const noexcept
{
	return m_activeSceneIndex;
}

void Core::GameManager::HandleEvents(const double p_dt)
{
	...
	if (inputManagerInstance->GetKeyDown(Rendering::Utils::Event<int>::KeyCode::F))
	{
		const auto& weaponComponentRef = GetActiveScene()->FindGameObject("player")->GetComponent<Core::Components::WeaponComponent>();
		if (weaponComponentRef)
		{
			weaponComponentRef->Shoot();
		}
	}
...
void Core::GameManager::Update(const double p_dt)
{
